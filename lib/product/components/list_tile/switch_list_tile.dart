...
class CustomSwitchListTile extends StatefulWidget {
  const CustomSwitchListTile({Key? key, this.title,required this.boolValue}) : super(key: key);
  final String? title;
  final bool boolValue;

  @override
  _CustomSwitchListTileState createState() => _CustomSwitchListTileState(boolValue);
}
...
class _CustomSwitchListTileState extends State<CustomSwitchListTile> {
  ...
  Widget build(BuildContext context) {
    return SwitchListTile(
      title: Text(widget.title ?? "",style: context.textTheme.titleMedium?.copyWith(color: Colors.black,fontWeight: FontWeight.w600),),
      value: visibility,
